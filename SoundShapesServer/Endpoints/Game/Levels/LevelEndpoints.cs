...
public class LevelEndpoints : EndpointGroup
{
    ...
    public ListResponse<LevelResponse>? GetLevels(RequestContext context, GameDatabaseContext database, GameUser? user,
        GameToken? token)
    {
        ...
        order ??= context.GetOrderType<LevelOrderType>() ?? LevelOrderType.CreationDate;

        PaginatedList<GameLevel> levels =
            database.GetPaginatedLevels((LevelOrderType)order, descending, filters, from, count, user);

        return new ListResponse<LevelResponse>(levels.Items.Select(l => new LevelResponse(l, user)), levels.TotalItems,
            from,
            count);
    }
}