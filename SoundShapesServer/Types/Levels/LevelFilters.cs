...
public class LevelFilters
{
    ...
    [FilterProperty("inLatestDaily", "Filter out levels that were not picked as a daily level on the date that the latest daily level was picked.")]
    public bool? InLatestDaily { get; set; }
    
    [FilterProperty("search", "Filter out levels whose name isn't included in query. Also includes levels where the author's username matches the query.")]
    ...
    [FilterProperty("uploadPlatforms", "Filter out levels that were not created on any of the specified upload platforms.")]
